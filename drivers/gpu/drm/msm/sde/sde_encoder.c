...
void sde_encoder_poll_for_framedone(struct drm_encoder *drm_enc)
{
	struct sde_encoder_virt *sde_enc = NULL;
	struct sde_encoder_phys *phys_enc;
	u32 loop_count = 20, i;
	u32 sleep = 16600 / loop_count;
	...
	for (i = 0; i < loop_count; i++) {
		if (atomic_read(&phys_enc->pending_retire_fence_cnt) == 0)
			break;
		usleep_range(sleep, sleep * 2);
	}
}

bool is_virtual_disp(struct drm_encoder *drm_enc)
{
	struct sde_encoder_virt *sde_enc = NULL;
