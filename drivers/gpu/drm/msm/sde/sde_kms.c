		...
		struct drm_atomic_state *old_state)
{
	...
	struct drm_encoder *encoder;
	struct drm_device *dev;
	int i, rc = 0, flag = false;
	short ref_cnt_remove = 0, ref_cnt_add = 0;
	...
	list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
		if (encoder->crtc && encoder->crtc->state &&
			 sde_encoder_is_primary_display(encoder) &&
	...
	if (flag && sde_kms->core_client->refcount != 0) {
		list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
			...
			if (!is_virtual_disp(encoder))
				continue;

			sde_encoder_poll_for_framedone(encoder);

			ref_cnt_remove = sde_kms->core_client->refcount;
			ref_cnt_add = ref_cnt_remove;
			pr_err("%s %d\n", __func__, ref_cnt_remove);
			while (ref_cnt_remove--)
				sde_power_resource_enable(&priv->phandle,
						 sde_kms->core_client, false);
			while (ref_cnt_add--)
				sde_power_resource_enable(&priv->phandle,
